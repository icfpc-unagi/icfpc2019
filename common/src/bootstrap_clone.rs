			...
			for _ in 0..k-1 {
				...
				for j in 1..ts.len() {
					for p in 0..clones.len() {
						if !used[p] {
							let act = if ps[j] == clones[p] {
								vec![]
							} else {
								bfs.search_fewest_actions_to_move(map, &PlayerState::new(ps[j].0, ps[j].1), clones[p].0, clones[p].1)
							};
				...
				ps[j] = clones[p];
				acts[j].extend(act);
				if p < clones.len() - buy_clone {
					while acts[0].len() <= ts[j] {
